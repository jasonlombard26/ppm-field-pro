...
window.openSiteV6=id=>{setActiveSite(id);setTimeout(()=>renderSiteDetailV6(id,'info'),0)};
window.renderSiteDetailV6=(id,tab='info')=>{
  ...
  }else if(tab==='headend')body=`<div class=card><h3 style="margin-top:0">Head End</h3><div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:10px"><div class=card style="margin:0"><h4 style="margin:0 0 8px">Access System</h4><div class="tiny muted">No records added yet.</div></div><div class=card style="margin:0"><h4 style="margin:0 0 8px">Intruder System</h4><div class="tiny muted">No records added yet.</div></div><div class=card style="margin:0"><h4 style="margin:0 0 8px">CCTV Server</h4><div class="tiny muted">No records added yet.</div></div><div class=card style="margin:0"><h4 style="margin:0 0 8px">Network Switches</h4><div class="tiny muted">No records added yet.</div></div><div class=card style="margin:0"><h4 style="margin:0 0 8px">Intercom System</h4><div class="tiny muted">No records added yet.</div></div></div></div>`;
...
window.renderSites=()=>{if(typeof oldRenderSites==='function')oldRenderSites();const p=$('sitePicker');if(p)p.innerHTML=db.sites.map(s=>`<option value="${s.id}" ${s.id===activeId()?'selected':''}>${esc(s.name)}</option>`).join('');if($('sites')?.classList.contains('active')){if($('sites').dataset.mode==='detail')renderSiteDetailV6(activeId(),'info');else showSitesList();}};
