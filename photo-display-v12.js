...
function assetIdFromButton(btn){
 const onclick=btn.getAttribute('onclick')||'';
 const standard=onclick.match(/editDeviceV7\((\d+),\s*['"](cctv|access)['"]\)/);
 if(standard)return standard[1];
 const intrusion=onclick.match(/intrusionDeviceFormV10\((\d+)\)/);
 return intrusion?intrusion[1]:'';
}
function decorate(){
 const sec=document.getElementById('sites');if(!sec)return;
 sec.querySelectorAll('button[onclick*="editDeviceV7("],button[onclick*="intrusionDeviceFormV10("]').forEach(btn=>{
  const assetId=assetIdFromButton(btn);if(!assetId)return;
