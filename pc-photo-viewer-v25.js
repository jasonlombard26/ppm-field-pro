...
function isPhoto(img){if(!img||img.tagName!=='IMG')return false;if(img.id==='ppmPcImgV25'||img.id==='ppmPhotoViewerImg')return false;return !!(img.dataset?.ppmPhotoId||img.closest('#sites,#photoGrid,#modalRoot,.modal,.modalBox,[role="dialog"]'));}
function openImg(img,e){if(!isPhoto(img))return false;if(e){e.preventDefault();e.stopPropagation();if(e.stopImmediatePropagation)e.stopImmediatePropagation();}open(img.currentSrc||img.src,img.title||img.alt||'Site photo');return true;}
function mark(){document.querySelectorAll('#sites img,#photoGrid img,#modalRoot img,.modal img,.modalBox img,[role="dialog"] img,img[data-ppm-photo-id]').forEach(img=>{if(!isPhoto(img))return;img.style.cursor='zoom-in';img.style.pointerEvents='auto';if(!img.dataset.ppmPcPointerBound){img.dataset.ppmPcPointerBound='1';img.addEventListener('pointerup',e=>{if(e.button!==undefined&&e.button!==0)return;openImg(img,e);},true);}});}
document.addEventListener('click',e=>{const img=e.target?.closest?.('img');openImg(img,e);},true);
new MutationObserver(()=>setTimeout(mark,0)).observe(document.documentElement,{childList:true,subtree:true});mark();setTimeout(mark,250);setTimeout(mark,1000);
})();
