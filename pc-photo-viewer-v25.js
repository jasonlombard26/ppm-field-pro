...
function mark(){document.querySelectorAll('#sites img,#photoGrid img,#modalRoot img,.modal img,.modalBox img,[role="dialog"] img,img[data-ppm-photo-id]').forEach(img=>{if(isPhoto(img))img.style.cursor='zoom-in';});}
// One delegated capture handler works for dynamically-created card and modal photos.
document.addEventListener('click',e=>{const img=e.target?.closest?.('img');if(!isPhoto(img))return;e.preventDefault();e.stopPropagation();e.stopImmediatePropagation();open(img.currentSrc||img.src,img.title||img.alt||'Site photo');},true);
