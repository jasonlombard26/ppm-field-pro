...
let timer=null,longPressed=false,currentPhoto=null,startX=0,startY=0;
...
function openMenu(photo){closeMenu();currentPhoto=photo;const wrap=document.createElement('div');wrap.id='ppmPhotoActionMenu';wrap.style.cssText='position:fixed;inset:0;z-index:100001;background:rgba(0,0,0,.5);display:flex;align-items:flex-end;justify-content:center;padding:16px;box-sizing:border-box';wrap.innerHTML=`<div style="background:white;color:#0f172a;border-radius:16px;padding:12px;width:min(420px,100%);box-shadow:0 20px 40px rgba(0,0,0,.28)"><div style="font-weight:700;padding:8px 10px 12px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${String(photo.label||'Site photo').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]))}</div><button type="button" data-action="rename" style="width:100%;padding:14px;border:0;border-top:1px solid #e2e8f0;background:white;text-align:left;font-size:17px">Rename</button><button type="button" data-action="share" style="width:100%;padding:14px;border:0;border-top:1px solid #e2e8f0;background:white;text-align:left;font-size:17px">Share</button><button type="button" data-action="delete" style="width:100%;padding:14px;border:0;border-top:1px solid #e2e8f0;background:white;text-align:left;font-size:17px;color:#b91c1c;font-weight:700">Delete</button><button type="button" data-action="cancel" style="width:100%;padding:14px;border:0;border-top:1px solid #e2e8f0;background:white;text-align:center;font-size:16px;font-weight:700">Cancel</button></div>`;document.body.appendChild(wrap);wrap.addEventListener('click',e=>{const a=e.target?.dataset?.action;if(!a){if(e.target===wrap)closeMenu();return;}if(a==='cancel')return closeMenu();if(a==='rename')return renamePhoto(photo);if(a==='share')return sharePhoto(photo);if(a==='delete')return deletePhoto(photo);});}
window.openPhotoActionsV13=id=>{const p=findPhoto(id);if(p)openMenu(p);else alert('Photo record could not be found.');};
...
function startPress(img,e){const p=findPhoto(img.dataset.ppmPhotoId);if(!p)return;longPressed=false;clearTimeout(timer);startX=e.clientX||0;startY=e.clientY||0;timer=setTimeout(()=>{longPressed=true;openMenu(p);if(navigator.vibrate)navigator.vibrate(30);},550);}
...
document.addEventListener('pointerup',cancelPress,true);document.addEventListener('pointercancel',cancelPress,true);document.addEventListener('pointermove',e=>{if(timer&&Math.abs((e.clientX||0)-startX)+Math.abs((e.clientY||0)-startY)>12)cancelPress();},true);
document.addEventListener('touchstart',e=>{const img=e.target?.closest?.('img[data-ppm-photo-id]');if(img&&e.touches?.[0])startPress(img,e.touches[0]);},{capture:true,passive:true});
document.addEventListener('touchend',cancelPress,true);document.addEventListener('touchcancel',cancelPress,true);
...
document.addEventListener('click',e=>{const menuBtn=e.target?.closest?.('button[data-ppm-photo-menu]');if(menuBtn){e.preventDefault();e.stopImmediatePropagation();window.openPhotoActionsV13(menuBtn.dataset.ppmPhotoMenu);return;}if(!longPressed)return;const img=e.target?.closest?.('img[data-ppm-photo-id]');if(img){e.preventDefault();e.stopImmediatePropagation();longPressed=false;}},true);
})();
