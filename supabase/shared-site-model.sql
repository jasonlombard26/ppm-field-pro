...
as $$
  select exists(
    select 1 from public.ppm_site_members m
...
as $$
  select exists(
    select 1 from public.ppm_site_members m
    where m.site_id = p_site_id and m.user_id = auth.uid() and m.role = 'admin'
  );
$$;
...
create policy "PPM shared sites select" on public.ppm_sites
for select to authenticated
using (public.ppm_user_has_site_access(id));

drop policy if exists "PPM shared sites insert" on public.ppm_sites;
create policy "PPM shared sites insert" on public.ppm_sites
for insert to authenticated
with check (created_by = auth.uid());

drop policy if exists "PPM shared sites update" on public.ppm_sites;
create policy "PPM shared sites update" on public.ppm_sites
for update to authenticated
using (public.ppm_user_has_site_access(id))
with check (public.ppm_user_has_site_access(id));

drop policy if exists "PPM shared sites delete" on public.ppm_sites;
create policy "PPM shared sites delete" on public.ppm_sites
for delete to authenticated
using (public.ppm_user_is_site_admin(id));

drop policy if exists "PPM memberships select" on public.ppm_site_members;
create policy "PPM memberships select" on public.ppm_site_members
for select to authenticated
...
with check (public.ppm_user_is_site_admin(site_id));
...
declare
  target_user uuid;
begin
  if not public.ppm_user_is_site_admin(p_site_id) then
    raise exception 'Only a site admin can grant access';
  end if;
  if p_role not in ('admin','technician','viewer') then
    raise exception 'Invalid role';
  end if;
  select id into target_user from auth.users where lower(email)=lower(trim(p_email)) limit 1;
  if target_user is null then
    raise exception 'No Supabase user exists for that email';
  end if;
  insert into public.ppm_site_members(site_id,user_id,role)
  values(p_site_id,target_user,p_role)
